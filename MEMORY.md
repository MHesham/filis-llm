...
- **Open WebUI settings live in its database, not the script.** These are seeded from the `-e` env vars in `start-webui.sh` **only on first-ever initialization** (empty DB). Once the DB exists, restarting the container with a changed env var — `docker rm -f` + fresh `docker run`, not just a process restart — does **not** update the stored value. Only the admin panel or the admin API changes it after that.
...
- **Nothing runs `/etc/init.d` scripts at boot.** Process 1 is `s6-svscan` from **s6-overlay**, which starts only services defined in `/etc/s6-overlay/s6-rc.d/` (Thunder's `containerd`, `dockerd`, `sshd`, `thundercompute-cdi-spec`). There's no systemd, cron, or `rc.local`.
  - **What happened (2026-09-14):** after a restart, `cloudflared` stayed `Stopped` and `https://<PUBLIC_DOMAIN>` returned Cloudflare 530, while the Docker containers came back (Docker restarts its own).
  - **Fix:** a longrun at `/etc/s6-overlay/s6-rc.d/cloudflared/` (`type` = `longrun`, `dependencies.d/base`, `run` = `exec /usr/bin/cloudflared --no-autoupdate tunnel run --token-file /etc/cloudflared/token`), plus an empty `/etc/s6-overlay/s6-rc.d/user/contents.d/cloudflared`. `/etc` is on the saved disk, so it survives restores.
  - **A bad entry there can break dockerd/sshd at boot.** Validate first: copy the tree to a temp dir, add the service, and run `s6-rc-compile <tmpdb> /package/admin/s6-overlay/etc/s6-rc/sources <tree>`; check `s6-rc-db -c <tmpdb> contents user`. Don't `s6-rc-update` the live tree.
  - **Not yet observed across a real reboot** as of 2026-09-14 — only compile-validated. After the next restart, confirm with `ls /run/service/cloudflared` and `curl -o /dev/null -w '%{http_code}' https://<PUBLIC_DOMAIN>`.
- **Node.js/npm are not preinstalled.** Needed for Qwen Code (and any other Node-based coding agent). Install via NodeSource: `curl -fsSL https://deb.nodesource.com/setup_22.x | sudo -E bash - && sudo apt-get install -y nodejs`.
- **Agentic coding verified live, 2026-09-13:** Qwen Code (`@qwen-code/qwen-code`) configured with `baseUrl: https://<PUBLIC_DOMAIN>/api/v1` and an Open WebUI personal API key, run with `--yolo` against a real "write a file, then execute it" task — both the file-write and shell-exec tool calls worked end to end through Open WebUI → vLLM.
